...
The automated setup allows us to command the severity of faults in **real-time**. Thus, in a streaming fashion, the faulty condition is actively **modulated** and **annotated**, giving users the chance to invesigate and validate time-varying fault **prediction** algorithms. (Arguably, traditional 'component-switching' style fault generation methods only grant users with out-of-the-box **diagnostic/detection** capacity because there isn't data BEFORE the fault takes place.)
